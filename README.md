...
- **Определение клиники** — автоматически определяет подразделение по ключевым словам в файле (`clinics.yaml`), добавляет колонку `Клиника`
...
- **Экспорт на сетевой диск** — ежедневный CSV + ежемесячный `master_YYYY-MM.csv` автоматически копируются в сетевую папку (CIFS/SMB) для загрузки в 1С
